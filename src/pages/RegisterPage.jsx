...
const formSchema = Joi.object({
  firstName: Joi.string().alphanum().min(6)
    .max(20)
    .required(),
  lastName: Joi.string().alphanum().min(6)
    .max(20)
    .required(),
  email: Joi.string()
  ...
  return (
    <div className="flex justify-center items-center" style={backgroundStyle}>
      <div className="w-[90%] min-w-[360px] lg:max-w-[921px] h-[780px] rounded-[10px] bg-gray-primary   px-[10%] semi-lg:px-[100px] flex flex-col">
        ...
        <form className="h-full py-4 semi-lg:py-8 flex flex-col justify-around " onSubmit={handleSubmitForm}>
          ...
          />
          {errorFormInfo.firstName && <p className="semi-lg:hidden text-red-500">{errorFormInfo.firstName}</p>}
          <input
            type="text"
